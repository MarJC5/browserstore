document.addEventListener('DOMContentLoaded', () => {
    ...
    const renderItems = (items, container, storageType) => {
        ...
        items.forEach(item => {
            ...
            deleteButton.className = "text-sm p-2 bg-red-500 text-white rounded hover:bg-red-400 transition-colors duration-300";
            deleteButton.textContent = "Delete";
            deleteButton.addEventListener('click', () => {
                deleteStorage(item.key, storageType);
                li.remove();
                updateTotalSize();
            });

            let previewButton = document.createElement('button');
            previewButton.className = "text-sm p-2 bg-blue-500 text-white rounded hover:bg-blue-400 transition-colors duration-300";
            ...
            previewButton.addEventListener('click', () => {
                ...
                if (typeof itemContent === 'string') {
                    try {
                        parsedContent = JSON.parse(itemContent);
                    } catch (e) {
                        parsedContent = itemContent; // Keep it as a string if parsing fails
                    }
                } else {
                    parsedContent = itemContent;
                }

                if (typeof parsedContent === 'object') {
                    preview.innerHTML = jsonViewer(parsedContent, true);
                } else {
                    preview.innerText = parsedContent;
                }
    ...
    const deleteStorage = (key, storageType) => {
        browser.runtime.sendMessage({ command: "deleteLocalStorage", key: key }, (response) => {
            if (response) {
    ...
    const updateTotalSize = () => {
        browser.runtime.sendMessage({ command: "getAllStorageDetails" }, (response) => {
            if (response) {
    ...
    browser.runtime.sendMessage({ command: "getLocalStorageDetails" }, (response) => {
        ...
        if (response) {
            ...
            renderItems(localStorage.details, localStorageList, 'localStorage');
            renderItems(sessionStorage.details, sessionStorageList, 'sessionStorage');
            ...
            searchInput.addEventListener('input', (e) => {
                ...
                renderItems(filteredLocalItems, localStorageList, 'localStorage');
                renderItems(filteredSessionItems, sessionStorageList, 'sessionStorage');
            });
        } else {
